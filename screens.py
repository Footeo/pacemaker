...
class Screens:
	...
	def mainScreen(self):  #After user has logged in they have access to buttons for navigation
		...
		self.step2 = Label(self.contentFrame,text="2. Input parameters").grid(row=4, column=2, padx=35,pady=10)
		...
		self.LRLLabel = Label(self.contentFrame, text = "Lower Rate Limit (BPM)").grid(row=5,column=0)
		self.LRLInput = Entry(self.contentFrame)
		self.LRLInput.grid(row=6,column=0,pady=10)
		self.URLLabel = Label(self.contentFrame, text="Upper Rate Limit (BPM)").grid(row=5,column=1)
		self.URLInput = Entry(self.contentFrame)
		self.URLInput.grid(row=6,column=1,pady=10)
		self.MaxSensorRateLabel = Label(self.contentFrame, text="Maximum Sensor Rate (BPM)").grid(row=5,column=2)
		self.MaxSensorRateInput = Entry(self.contentFrame)
		self.MaxSensorRateInput.grid(row=6,column=2,pady=10)
		self.FixedAVDelayLabel = Label(self.contentFrame, text="Fixed AV Delay (V)").grid(row=5,column=3)
		self.FixedAVDelayInput = Entry(self.contentFrame)
		self.FixedAVDelayInput.grid(row=6,column=3,pady=10)
		self.AtrAmplitudeLabel = Label(self.contentFrame, text="Atrial Amplitude (V)").grid(row=5,column=4)
		self.AtrAmplitudeInput = Entry(self.contentFrame)
		self.AtrAmplitudeInput.grid(row=6,column=4,pady=10)
		self.VentAmplitudeLabel = Label(self.contentFrame, text="Ventricular Amplitude (V)").grid(row=7,column=0)
		self.VentAmplitudeInput = Entry(self.contentFrame)
		self.VentAmplitudeInput.grid(row=8,column=0)
		self.AtrPulseWidthLabel = Label(self.contentFrame, text="Atrial Pulse Width (ms)").grid(row=7,column=1)
		self.AtrPulseWidthInput = Entry(self.contentFrame)
		self.AtrPulseWidthInput.grid(row=8,column=1)
		self.VentPulseWidthLabel = Label(self.contentFrame, text="Ventricular Pulse Width (ms)").grid(row=7,column=2)
		self.VentPulseWidthInput = Entry(self.contentFrame)
		self.VentPulseWidthInput.grid(row=8,column=2)
		self.AtrSenseLabel = Label(self.contentFrame, text="Atrial Amplitude (V)").grid(row=7,column=3)
		self.AtrSenseInput = Entry(self.contentFrame)
		self.AtrSenseInput.grid(row=8,column=3)
		self.VentSenseLabel = Label(self.contentFrame, text="Ventricular Amplitude (V)").grid(row=7,column=4)
		self.VentSenseInput = Entry(self.contentFrame)
		self.VentSenseInput.grid(row=8,column=4)
		self.VRP = Label(self.contentFrame, text="Ventricular Refractory Period (ms)").grid(row=9,column=0)
		self.VRPInput = Entry(self.contentFrame)
		self.VRPInput.grid(row=10,column=0)
		self.ARP = Label(self.contentFrame, text="Atrial Refractory Period (ms)").grid(row=9,column=1)
		self.ARPInput = Entry(self.contentFrame)
		self.ARPInput.grid(row=10,column=1)
		self.PVARP = Label(self.contentFrame, text="Post Ventricular Atrial Refractory Period (ms)").grid(row=9,column=2)
		self.PVARPInput = Entry(self.contentFrame)
		self.PVARPInput.grid(row=10,column=2)
		self.RateSmoothingLabel = Label(self.contentFrame, text="Rate Smoothing (%)").grid(row=9,column=3)
		...
		self.RateSmoothingInput.grid(row=10,column=3)
		self.ActivityThresholdLabel = Label(self.contentFrame, text="Activity Threshold ").grid(row=9,column=4)
		...
		self.ActivityThresholdInput.grid(row=10,column=4)
		self.ReactionTimeLabel = Label(self.contentFrame, text="Reaction Time (ms)").grid(row=11,column=0)
		self.ReactionTimeInput = Entry(self.contentFrame)
		self.ReactionTimeInput.grid(row=12,column=0)
		self.ResponseFactorLabel = Label(self.contentFrame, text="Response Factor").grid(row=11,column=1)
		self.ResponseFactorInput = Entry(self.contentFrame)
		self.ResponseFactorInput.grid(row=12,column=1)
		self.RecoveryTimeLabel = Label(self.contentFrame, text="Recovery Time (min)").grid(row=11,column=2)
		self.RecoveryTimeInput = Entry(self.contentFrame)
		self.RecoveryTimeInput.grid(row=12,column=2)


		who = StringVar()
		who.set("Currently connected to "+settings.user+"'s pacemaker")
		self.deviceText1 = Label(self.optionsFrame, textvariable=who, fg="green").grid(row=10,sticky="S",pady=10)  # Have to change this value based on the users device
	...
	def AOO(self):  #When one of these buttons are pressed there should be an identifier number so it knows how to save the parameters

		self.LRLInput.configure(bg="white")
		self.URLInput.configure(bg="white")
		self.MaxSensorRateInput.configure(bg="grey")
		self.FixedAVDelayInput.configure(bg="grey")
		self.AtrAmplitudeInput.configure(bg="white")
		self.VentAmplitudeInput.configure(bg="grey")
		self.AtrPulseWidthInput.configure(bg="white")
		self.VentPulseWidthInput.configure(bg="grey")
		self.AtrSenseInput.configure(bg="grey")
		self.VentSenseInput.configure(bg="grey")
		self.VRPInput.configure(bg="grey")
		self.PVARPInput.configure(bg="grey")
		self.ARPInput.configure(bg="grey")
		self.RateSmoothingInput.configure(text="N/A")
		self.ActivityThresholdInput.configure(text="N/A")
		self.ReactionTimeInput.configure(bg="grey")
		self.RecoveryTimeInput.configure(bg="grey")
		self.ResponseFactorInput.configure(bg="grey")
	...
	def AAI(self):
		...
		self.AtrPulseWidthInput.configure(bg="white")
		self.VentPulseWidthInput.configure(bg="grey")
		self.AtrSenseInput.configure(bg="white")
		self.VentSenseInput.configure(bg="grey")
		self.VRPInput.configure(bg="grey")
		self.PVARPInput.configure(bg="white")
		self.ARPInput.confiure(bg="white")
		# self.RateSmoothingInput.configure(bg="grey")
		# self.ActivityThresholdInput.configure(bg="grey")
		self.ReactionTimeInput.configure(bg="grey")
		self.RecoveryTimeInput.configure(bg="grey")
		self.ResponseFactorInput.configure(bg="grey")
